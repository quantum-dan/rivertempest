...
A full example notebook for running real-time, contiguous US forecasts
for 1-7 days is available on the RiverTempest website at
https://rivertempest.org/next/forecast.html, with source code and full
description (plus links to outputs) on
`GitHub <https://github.com/mines-ciroh/natl-temp-forecast>`__.
...
For forecast data, past and present HRRR works (except, in historical
data, for humidity, which must be retrieved through Gridmet), but has
become very slow for unknown reasons. It is quick enough for actual
forecasting use, but for historical (coefficient estimation) data, I
recommend using a shorter sample of HRRR data to adjust Gridmet
climatology. Gridmet is far faster.
